...
當處理 Blockly 產生器 (.js) 與產出的 Processing 代碼 (.pde) 時，必須嚴格遵守以下規範：
1.  **產生器 JS 中的字串與換行**：
    - **實體換行禁止**：在單引號 `'` 或雙引號 `"` 定義的字串中，絕對禁止出現實體換行。若需換行，必須寫成 `\\n` (透過工具寫入時) 或 `\n` (執行時)。
    - **模板字串**：在 JS 中使用反引號 `` ` `` (Template Literals) 時，可以使用實體換行，這在注入長篇 Java 類別時最為安全。
2.  **Java (PDE) 代碼產出規範**：
    - **代碼換行**：產生器 `return` 的字串中，若要讓生成的 `.pde` 檔案換行，請在字串結尾加上 `\n`。
    - **Java 正規表達式**：在 Java 的 `split` 中使用的反斜線（如 `split("\\+")`），在產生器 JS 中必須寫成 `split("\\\\\\\\+")` (雙重轉義)。
3.  **三層字串意識**：
    - 第一層：Gemini API Payload。
    - 第二層：磁碟上的 `.js` 檔案。
    - 第三層：`.js` 執行後 `return` 給 Blockly 的字串（最終變為 `.pde`）。

...
- **[架構演進] 視覺模組與產生器深度拆分完成 (8-2)**：
    - 拆分 \locks/visual.js\ 為 \isual_core.js\、\isual_geometry.js\ 與 \isual_transform.js\。
    - 同步拆分對應的產生器，解決單一檔案過大且邏輯混雜問題。
    - 將顏色轉換輔助函式 (\hexToJavaColor\, \hexToHue\) 移至 \utils.js\。
- **[架構演進] 產生器框架與 Java 代碼庫分離 (8-3)**：
    - 建立 \media/generators/java_libs.js\，統一收納 UGen、MelodicSampler 等長篇 Java 類別與輔助方法模板。
    - \_core.js\ 瘦身完成，僅保留 Blockly 到 Java 的轉換框架邏輯。
- **[架構演進] 前端 Webview 邏輯模組化 (8-4)**：
    - 拆分 \main.js\ 為 \scode_bridge.js\ (通訊)、\lockly_manager.js\ (核心操作) 與 \event_handlers.js\ (互動/監聽)。
    - 使用 ES 模組架構 (\import\/\export\) 重構進入點 \main.js\。

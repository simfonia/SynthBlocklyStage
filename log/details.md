...
# 2026-02-07 技術細節：智慧聯動與穩定性強化

### 1. 動態下拉選單的「時位」問題
- **細節**：在 `sb_set_effect_param` 中，下拉選單內容是動態生成的。若使用者直接在 `sb_instrument_container` 修改樂器名稱，已放置的「更新參數」積木選單不會主動更新。
- **對策**：實作 `getEffectOptions` 函式，每次使用者「點擊選單」時才掃描工作區。這避開了複雜的事件監聽，且保證選單內容始終為最新狀態。

### 2. 參數更新的「反射路徑」優化
- **問題**：初版反射更新嘗試存取 `.value` 欄位，但在不同 Minim 版本中，`UGenInput` 可能沒有該欄位。
- **解決方案**：改為呼叫 `setLastValue(float)` 方法。這是 Minim 官方推薦的參數即時更新方式，支援所有 UGen 輸入項（如 Frequency, Resonance, Amplitude 等）。

### 3. Default 樂器的「立體聲崩潰」
- **坑點**：未經 Container 定義的 `default` 樂器若直接被賦予相位 (Pan)，會報 `Pan MUST be ticked with STEREO output`。
- **對策**：在 `checkMainMixer` 中預設建立一個「虛擬立體聲源 (Dummy Pan)」，強制 `mainMixer` 進入立體聲模式。並在啟動時預先呼叫 `getInstrumentMixer("default")` 進行預熱。

### 4. Serial 事件的換行同步
- **細節**：如果 Arduino 傳送資料速度過快，且沒設定緩衝邊界，`serialEvent` 會因為解析不全導致 Java 噴錯。
- **對策**：在舞台端的 `serialInputDevice` 輔助函式中強制加入 `bufferUntil('\n')`。這確保了不論使用者在哪裡（積木或舞台 UI）啟動 Serial，都能獲得穩定的字串讀取。

### 5. 擴充功能樣板與狀態同步
- **技術點**：VS Code Webview 載入 XML 後預設會重置 `isDirty`。若想讓預設樣板顯示為「未儲存」，必須在 `initializeWorkspace` 訊息中帶入 `isTemplate` 標記，並在前端手動設定 `isDirty = true`。

# 2026-02-08 技術細節：UI 排版革命與效能極限優化

### 1. 原子化翻譯鍵值 (Atomic Translation Keys)
- **坑點**：使用 `.split('%1')` 手動切割長字串標籤雖然靈活，但極度依賴語序，且在翻譯變動時容易崩潰。
- **對策**：全面重構語言檔，將單一訊息拆分為 `_INST`, `_PITCH`, `_VEL` 等獨立標籤。
- **優勢**：積木定義改為直接引用這些原子鍵值，徹底解決了多語系對齊問題，並支援更穩健的橫向/縱向排版切換。

### 2. 外科手術式積木更新 (Surgical Updates)
- **技術點**：孤兒積木檢查 (`updateOrphanBlocks`) 若每次都全量重設狀態，會引發大量 DOM 變動導致畫面閃爍與 Lag。
- **實作**：引入 `hasDisabledReason('orphan')` 比對。只有在積木「當前狀態」與「計算後的目標狀態」不符時，才呼叫 `setDisabledReason`。這將 UI 刷新負擔降到了最低。

### 3. 拖曳中的「邏輯禁飛區」 (Drag-Skip Policy)
- **問題**：拖曳積木時，Blockly 每秒會發送數十次 `BLOCK_MOVE` 事件。若在這些事件中執行按鍵衝突或孤兒檢查，會卡住主執行緒，導致垃圾桶掀蓋動畫卡頓。
- **方案**：在監聽器頂端加入 `if (workspace.isDragging()) return;`。所有邏輯檢查統一移入 `setTimeout` 防抖計時器（100ms），確保使用者「放下積木」後才進行運算。

### 4. 雙模音高輸入 (Hybrid Pitch Input)
- **細節**：為了同時支援 MIDI 數字與音名，插槽類型設為 `['Number', 'String']`。
- **轉型機制**：在 `_core.js` 實作 `getMidi(Object o)`。Java 端會判斷輸入是否為 `Number`，若非則視為字串並呼叫 `noteToMidi()`。這統一了編曲類與演奏類積木的輸入規範。

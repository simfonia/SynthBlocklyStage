...
void serialEvent(Serial p) {
  ...
  if (floatVal(key_id_var) == floatVal(1)) {
    ...
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal("Piano", "Dm", ms, floatVal(100));
    }
  } else if (floatVal(key_id_var) == floatVal(3)) {
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal("Piano", "Em", ms, floatVal(100));
    }
  } else if (floatVal(key_id_var) == floatVal(4)) {
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal(currentInstrument, "F", ms, floatVal(100));
    }
  } else if (floatVal(key_id_var) == floatVal(5)) {
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal("Piano", "G", ms, floatVal(100));
    }
  } else if (floatVal(key_id_var) == floatVal(6)) {
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal("Piano", "Am", ms, floatVal(100));
    }
  } else if (floatVal(key_id_var) == floatVal(7)) {
    { 
      Object dVal = "4n";
      float ms = (dVal instanceof Number) ? ((Number)dVal).floatValue() : durationToMs(dVal.toString());
      playChordByNameInternal("Piano", "Bdim", ms, floatVal(100));
    }
  }

...
void setup() {
  ...
  if (!instrumentADSR.containsKey("DefaultSynth")) instrumentADSR.put("DefaultSynth", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    ...
    {
      if (instrumentFilters.containsKey("DefaultSynth")) { updateFilter("DefaultSynth", (float)20000, (float)0.5); }
      else { UGen prev = (UGen)instrumentEffectEnds.getOrDefault("DefaultSynth", getInstrumentMixer("DefaultSynth")); prev.unpatch(((UGen)instrumentPans.getOrDefault("DefaultSynth", getInstrumentMixer("DefaultSynth")))); UGen f = new MoogFilter((float)20000, constrain((float)0.5, 0.0f, 0.9f)); instrumentFilters.put("DefaultSynth", f); prev.patch(f).patch(((UGen)instrumentPans.getOrDefault("DefaultSynth", getInstrumentMixer("DefaultSynth")))); instrumentEffectEnds.put("DefaultSynth", f); }
    }
}

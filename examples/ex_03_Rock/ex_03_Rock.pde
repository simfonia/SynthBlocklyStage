                  ...
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("Kick", "").equals("DRUM")) {
                      if (token.equalsIgnoreCase("x")) {
                         ...
                         samplerGainMap.get("Kick").setValue(map(floatVal(110) * volScale, 0, 127, -40, 0));
                         samplerMap.get("Kick").trigger();
                      }
                    } else {
                      if (false) {
                        if (token.equals("x")) token = "C";
                        if (chords.containsKey(token)) playChordByNameInternal("Kick", token, noteDur * 0.9f, floatVal(110));
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, floatVal(110), noteDur * 0.9f); }
                      } else {
                        if (token.equalsIgnoreCase("x")) playNoteForDuration("Kick", 60, floatVal(110), noteDur * 0.8f);
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, floatVal(110), noteDur * 0.9f); }
                      }
                    }
                  }
                  ...
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("Snare", "").equals("DRUM")) {
                      if (token.equalsIgnoreCase("x")) {
                         ...
                         samplerGainMap.get("Snare").setValue(map(floatVal(100) * volScale, 0, 127, -40, 0));
                         samplerMap.get("Snare").trigger();
                      }
                    } else {
                      if (false) {
                        if (token.equals("x")) token = "C";
                        if (chords.containsKey(token)) playChordByNameInternal("Snare", token, noteDur * 0.9f, floatVal(100));
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Snare", midi, floatVal(100), noteDur * 0.9f); }
                      } else {
                        if (token.equalsIgnoreCase("x")) playNoteForDuration("Snare", 60, floatVal(100), noteDur * 0.8f);
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Snare", midi, floatVal(100), noteDur * 0.9f); }
                      }
                    }
                  }
                  ...
                  if (!token.equals("-")) {
                    if (instrumentMap.getOrDefault("HiHat", "").equals("DRUM")) {
                      if (token.equalsIgnoreCase("x")) {
                         ...
                         samplerGainMap.get("HiHat").setValue(map(floatVal(50) * volScale, 0, 127, -40, 0));
                         samplerMap.get("HiHat").trigger();
                      }
                    } else {
                      if (false) {
                        if (token.equals("x")) token = "C";
                        if (chords.containsKey(token)) playChordByNameInternal("HiHat", token, noteDur * 0.9f, floatVal(50));
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("HiHat", midi, floatVal(50), noteDur * 0.9f); }
                      } else {
                        if (token.equalsIgnoreCase("x")) playNoteForDuration("HiHat", 60, floatVal(50), noteDur * 0.8f);
                        else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("HiHat", midi, floatVal(50), noteDur * 0.9f); }
                      }
                    }
                  }

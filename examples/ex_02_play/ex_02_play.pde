...
HashMap<String, float[]> instrumentADSR = new HashMap<String, float[]>();
...
void setup() {
  ...
  instrumentADSR.put("default", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
...
void draw() {
  ...
  if (activeViews > 0) {
    ...
    if (showADSR) {
      ...
      float yS = 400 - (adsrS * 400 * 0.7); float yP = (adsrD > 0 || adsrS < 1.0) ? 400 * 0.2 : yS; float yB = 400 * 0.9;

...
void A() {
    new Thread(new Runnable() {
      public void run() {
        ...
        try { float beatMs = 60000.0f / bpm;
              float measureMs = beatMs * 4.0f;
              Thread.sleep((long)(((1-1) * measureMs))); } catch(Exception e) {}
        String rawPattern = "x--- --x- --x- x---";
        ArrayList<String> parsed = new ArrayList<String>();
        StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) {
          char c = rawPattern.charAt(j);
          if (c == ' ') {
            if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); }
          } else if (c == '.' || c == '-' || c == '|') {
            if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); }
            if (c != '|') parsed.add(String.valueOf(c));
          } else {
            buf.append(c);
          }
        }
        ...
        for (int k=0; k<steps.length; k++) {
          String token = steps[k].trim();
          if (token.equals(".") || token.equals("-")) {
            try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue;
          }
          int sustainSteps = 1;
          for (int next=k+1; next<steps.length; next++) {
            if (steps[next].trim().equals("-")) sustainSteps++;
            else break;
          }
    ...
    new Thread(new Runnable() {
      public void run() {
        ...
        try { float beatMs = 60000.0f / bpm;
              float measureMs = beatMs * 4.0f;
              Thread.sleep((long)(((2-1) * measureMs))); } catch(Exception e) {}
        String rawPattern = "x--- --x- --x- x---";
        ArrayList<String> parsed = new ArrayList<String>();
        StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) {
    ...
    new Thread(new Runnable() {
      public void run() {
        ...
        try { float beatMs = 60000.0f / bpm;
              float measureMs = beatMs * 4.0f;
              Thread.sleep((long)(((3-1) * measureMs))); } catch(Exception e) {}
    ...
    new Thread(new Runnable() {
      public void run() {
        ...
        try { float beatMs = 60000.0f / bpm;
              float measureMs = beatMs * 4.0f;
              Thread.sleep((long)(((4-1) * measureMs))); } catch(Exception e) {}

...
void A() {
    new Thread(new Runnable() {
      public void run() {
        ...
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((1-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "x--- --x- --x- x---"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
          float noteDur = stepMs * sustainSteps; if (token.length() > 0) {
            if (instrumentMap.getOrDefault("Clap", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Clap", 1.0f); samplerGainMap.get("Clap").setValue(map(100 * volScale, 0, 127, -40, 0)); samplerMap.get("Clap").trigger(); } }
            else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Clap", token, noteDur * 0.9f, (float)100); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Clap", midi, (float)100, noteDur * 0.9f); } }
            else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Clap", 60, (float)100, noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Clap", midi, (float)100, noteDur * 0.9f); } } }
          }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((1-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "Dm7... Dm7--- ..Dm7. ..Dm7."; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
          float noteDur = stepMs * sustainSteps; if (token.length() > 0) {
            if (instrumentMap.getOrDefault("Piano", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Piano", 1.0f); samplerGainMap.get("Piano").setValue(map(100 * volScale, 0, 127, -40, 0)); samplerMap.get("Piano").trigger(); } }
            else { if (true) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Piano", token, noteDur * 0.9f, (float)100); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Piano", midi, (float)100, noteDur * 0.9f); } }
            else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Piano", 60, (float)100, noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Piano", midi, (float)100, noteDur * 0.9f); } } }
          }
          try { Thread.sleep((long)stepMs); } catch (Exception e) {}
        }
      }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((1-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "D2--- ---- A2--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
          float noteDur = stepMs * sustainSteps; if (token.length() > 0) {
            if (instrumentMap.getOrDefault("Bass", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Bass", 1.0f); samplerGainMap.get("Bass").setValue(map(100 * volScale, 0, 127, -40, 0)); samplerMap.get("Bass").trigger(); } }
            else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Bass", token, noteDur * 0.9f, (float)100); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Bass", midi, (float)100, noteDur * 0.9f); } }
            else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Bass", 60, (float)100, noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Bass", midi, (float)100, noteDur * 0.9f); } } }
          }
          try { Thread.sleep((long)stepMs); } catch (Exception e) {}
        }
      }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((2-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "x--- --x- --x- x---"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
          float noteDur = stepMs * sustainSteps; if (token.length() > 0) {
            if (instrumentMap.getOrDefault("Kick", "").equals("DRUM")) { if (token.equalsIgnoreCase("x")) { float volScale = instrumentVolumes.getOrDefault("Kick", 1.0f); samplerGainMap.get("Kick").setValue(map(150 * volScale, 0, 127, -40, 0)); samplerMap.get("Kick").trigger(); } }
            else { if (false) { if (token.equals("x")) token = "C"; if (chords.containsKey(token)) playChordByNameInternal("Kick", token, noteDur * 0.9f, (float)150); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, (float)150, noteDur * 0.9f); } }
            else { if (token.equalsIgnoreCase("x")) playNoteForDuration("Kick", 60, (float)150, noteDur * 0.8f); else { int midi = noteToMidi(token); if (midi >= 0) playNoteForDuration("Kick", midi, (float)150, noteDur * 0.9f); } } }
          }
          try { Thread.sleep((long)stepMs); } catch (Exception e) {}
        }
      }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((2-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "..G7. ..G7. ---- G7..."; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((2-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "G1--- ---- D2--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((3-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "CM7... CM7--- ..CM7. ..CM7."; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((3-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "C2--- ---- G2--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((4-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "..A7. ..A7. ---- A7..."; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((4-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "A1--- ---- E2--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
...
void B() {
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((1-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "Dm7--- ---- G7--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((1-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "D2--- ---- G1--- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((2-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "CM7_End--- ---- ---- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
    ...
    new Thread(new Runnable() {
      public void run() {
        int timeout = 0; while(isCountingIn && timeout < 500) { try { Thread.sleep(10); timeout++; } catch(Exception e) {} }
        try { float beatMs = 60000.0f / bpm; float measureMs = beatMs * 4.0f; Thread.sleep((long)(((2-1) * measureMs))); } catch (Exception e) {}
        String rawPattern = "C2--- ---- ---- ----"; ArrayList<String> parsed = new ArrayList<String>(); StringBuilder buf = new StringBuilder();
        for (int j=0; j<rawPattern.length(); j++) { char c = rawPattern.charAt(j); if (c == ' ') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } } else if (c == '.' || c == '-' || c == '|') { if (buf.length() > 0) { parsed.add(buf.toString()); buf.setLength(0); } if (c != '|') parsed.add(String.valueOf(c)); } else { buf.append(c); } } if (buf.length() > 0) parsed.add(buf.toString());
        String[] steps = parsed.toArray(new String[0]); float stepMs = (60000.0f / bpm) / 4.0f;
        for (int k=0; k<steps.length; k++) { String token = steps[k].trim(); if (token.equals(".") || token.equals("-")) { try { Thread.sleep((long)stepMs); } catch (Exception e) {} continue; }
          int sustainSteps = 1; for (int next=k+1; next<steps.length; next++) { if (steps[next].trim().equals("-")) sustainSteps++; else break; }
...
void setup() {
  size(1600, 600);
    ...
    if (!instrumentMap.containsKey("Sax")) instrumentMap.put("Sax", "TRIANGLE");
  if (!instrumentADSR.containsKey("Sax")) instrumentADSR.put("Sax", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    instrumentMap.put("Sax", "SAW");
  ...
  if (!instrumentADSR.containsKey("Piano")) instrumentADSR.put("Piano", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    if (!melodicSamplers.containsKey("Piano")) melodicSamplers.put("Piano", new MelodicSampler(minim, "Piano"));
    ...
    instrumentADSR.put("Piano", new float[]{(float)0.01, (float)0.1, (float)0.5, (float)0.3});
  ...
  if (!instrumentADSR.containsKey("CHiHat")) instrumentADSR.put("CHiHat", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    samplerMap.put("CHiHat", new ddf.minim.ugens.Sampler("drum/ch.wav", 20, minim));
    samplerGainMap.put("CHiHat", new Gain(0.f));
    ((ddf.minim.ugens.Sampler)samplerMap.get("CHiHat")).patch((Gain)samplerGainMap.get("CHiHat")).patch(getInstrumentMixer("CHiHat"));
  ...
  if (surface.getNative() instanceof java.awt.Canvas) { ((java.awt.Canvas)surface.getNative()).requestFocus(); }
    ...
    chords.put("A7", new String[]{"A3", "C#4", "E4", "G4"});
    chords.put("CM7_End", new String[]{"C3", "G3", "C4", "E4", "G4", "B4"});
    new Thread(new Runnable() {
      public void run() {
        activeMelodyCount++;

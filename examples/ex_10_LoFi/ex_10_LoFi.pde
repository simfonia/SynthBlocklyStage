...
void setup() {
  ...
  if (!instrumentADSR.containsKey("MySynth")) instrumentADSR.put("MySynth", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    instrumentMap.put("MySynth", "MIXED");
    instrumentMixConfigs.put("MySynth", "SAW,PINK," + floatVal(10) + "," + floatVal(0) + "," + floatVal(0) + "," + floatVal(0));
    {
      if (instrumentPitchMods.containsKey("MySynth")) { UGen lfo = (UGen)instrumentPitchModLFOs.get("MySynth"); if (lfo instanceof ddf.minim.ugens.Oscil) { ((ddf.minim.ugens.Oscil)lfo).setFrequency((float)2); ((ddf.minim.ugens.Oscil)lfo).setAmplitude((float)50/1200.0f); } else if (lfo instanceof ddf.minim.ugens.Noise) { ((ddf.minim.ugens.Noise)lfo).amplitude.setLastValue((float)50/240.0f); } }
      else { UGen prev = (UGen)instrumentEffectEnds.getOrDefault("MySynth", getInstrumentMixer("MySynth")); prev.unpatch(((UGen)instrumentPans.getOrDefault("MySynth", getInstrumentMixer("MySynth")))); ddf.minim.ugens.TickRate tr = new ddf.minim.ugens.TickRate(1.0f); UGen lfo; if ("NOISE".equals("NOISE")) lfo = new ddf.minim.ugens.Noise((float)50/240.0f, ddf.minim.ugens.Noise.Tint.WHITE); else lfo = new ddf.minim.ugens.Oscil((float)2, (float)50/1200.0f, Waves.SINE); ddf.minim.ugens.Summer s = new SBSummer(); new ddf.minim.ugens.Constant(1.0f).patch(s); lfo.patch(s).patch(tr.value); instrumentPitchMods.put("MySynth", tr); instrumentPitchModLFOs.put("MySynth", lfo); prev.patch(tr).patch(((UGen)instrumentPans.getOrDefault("MySynth", getInstrumentMixer("MySynth")))); instrumentEffectEnds.put("MySynth", tr); }
    }
    {
      if (instrumentAutoFilters.containsKey("MySynth")) { ddf.minim.ugens.Oscil lfo = (ddf.minim.ugens.Oscil)instrumentAutoFilterLFOs.get("MySynth"); if (lfo != null) { lfo.setFrequency((float)0.5); lfo.setAmplitude(1000.0f * (float)20/100.0f); } ddf.minim.ugens.MoogFilter f = (ddf.minim.ugens.MoogFilter)instrumentAutoFilters.get("MySynth"); if (f != null) { f.resonance.setLastValue(constrain((float)0.4, 0.0f, 0.9f)); } }
      else { UGen prev = (UGen)instrumentEffectEnds.getOrDefault("MySynth", getInstrumentMixer("MySynth")); prev.unpatch(((UGen)instrumentPans.getOrDefault("MySynth", getInstrumentMixer("MySynth")))); ddf.minim.ugens.MoogFilter f = new ddf.minim.ugens.MoogFilter(1000, (float)0.4); ddf.minim.ugens.Oscil lfo = new ddf.minim.ugens.Oscil((float)0.5, 1000.0f * (float)20/100.0f, Waves.SINE); ddf.minim.ugens.Summer s = new SBSummer(); new ddf.minim.ugens.Constant(1000).patch(s); lfo.patch(s).patch(f.frequency); instrumentAutoFilters.put("MySynth", f); instrumentAutoFilterLFOs.put("MySynth", lfo); prev.patch(f).patch(((UGen)instrumentPans.getOrDefault("MySynth", getInstrumentMixer("MySynth")))); instrumentEffectEnds.put("MySynth", f); }
    }
}

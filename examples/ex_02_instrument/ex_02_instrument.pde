...
void setup() {
  ...
  if (!instrumentADSR.containsKey("Horn")) instrumentADSR.put("Horn", new float[]{defAdsrA, defAdsrD, defAdsrS, defAdsrR});
    ...
    harmonicPartials.put("Horn", new float[]{1f, 0.8f, 0.5f, 0.3f, 0.1f});
  ...
  if (surface.getNative() instanceof java.awt.Canvas) { ((java.awt.Canvas)surface.getNative()).requestFocus(); }
    ...
    {
      if (instrumentLimiters.containsKey("Master")) { ((SBCompressor)instrumentLimiters.get("Master")).setParams((float)(-3), 20.0f, (float)0.001, (float)0.1, 0.0f); }
      else { UGen prev = masterEffectEnd; prev.unpatch(masterGainUGen); SBCompressor c = new SBCompressor(); c.setParams((float)(-3), 20.0f, (float)0.001, (float)0.1, 0.0f); instrumentLimiters.put("Master", c); prev.patch(c).patch(masterGainUGen); masterEffectEnd = c; }
    }
}

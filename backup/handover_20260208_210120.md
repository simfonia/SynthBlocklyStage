...
1. 本次完成重點 (即時演奏優化與 UI 排版全面改進)：
- **排版架構原子化 (Major)**：
    - 徹底捨棄脆弱的 `.split('%1')` 解析邏輯，改用「原子化翻譯鍵值 (Atomic Keys)」。
    - 重構所有具備樂器選單的積木 (`演奏單音`, `播放和弦`, `播放旋律`, `音量`, `相位`, `音序器`)。
    - 實作「標籤與輸入緊密綁定」結構，確保在多語系（特別是英文長句子）下文字對齊精確且支援美觀的自動換行。
- **即時演奏系統強化**：
    - 演奏持續音積木加入「雙模音高」支援：同時接受 MIDI 編號 (60) 與音名 ('C4')。
    - 電腦鍵盤事件積木 (`ui_key_event`) 新增「按下 / 放開」模式切換，支援完整的 ADSR 即時控制。
- **核心效能與穩定性 (Critical)**：
    - **拖曳零延遲**：實作正在拖曳中 (`isDragging`) 自動跳過昂貴邏輯檢查的機制，恢復垃圾桶掀蓋動畫的 60fps 流暢度。
    - **嚴格孤兒限制**：實作外科手術式積木檢查，非帽子/容器積木放在頂層將自動失效（變灰），確保程式碼結構合法。
    - **崩潰修正**：透過 `setTimeout` 解決 Webview 失去焦點時手勢清除導致的 `start the same gesture twice` 崩潰問題。
- **雙專案同步**：將效能優化與邊界崩潰修正同步套用於 `#SynthBlockly`，維持兩大專案的開發穩定性。

2. 待辦任務 (Next Steps)：
- **壓力測試**：針對極高頻率的「按下/放開」操作進行執行緒安全性驗證。
- **範例重構**：利用新版的「放開」事件與「原子排版」重寫既有範例，提升教學品質。
- **大師級 Master Limiter**：規劃於 `checkMainMixer` 預設掛載總輸出限制器。
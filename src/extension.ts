...
class SynthBlocklyPanel {
    ...
    private static readonly DEFAULT_XML = `
<xml xmlns="https://developers.google.com/blockly/xml">
  ...
  <block type="sb_instrument_container" id="?IrjDlT{CuGc-;}sQao9" x="490" y="30">
    ...
    private _handleWebviewReady() {
        // If we don't have a path, it means it's a fresh start. Load the template.
        if (!this._currentXmlPath) {
            this._panel.webview.postMessage({ 
                command: 'initializeWorkspace', 
                xml: SynthBlocklyPanel.DEFAULT_XML,
                isTemplate: true,
                fileName: "",
                fullPath: ""
            });
    ...
    private async _handleOpenHelp(url?: string, fileName?: string) {
        try {
    ...
    private async _handleSetProcessingPath() {
    ...
    private _handleAutoSave(data: any) {
    ...
    private async _handleSaveProject(xml: string, code: string): Promise<string | undefined> {
        ...
        if (fileUri) {
            ...
            let finalXmlPath: string;
            let finalPdePath: string;
